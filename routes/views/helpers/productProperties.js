...
function filterValues (obj,key,currObj,end){
  if (typeof obj[key] === 'number' && obj[key] < currObj[key]) {
    (obj[key] = currObj[key]);
  }
  if (typeof obj[key] === 'string' && typeof currObj[key] === 'string' && obj[key].indexOf(currObj[key]) < 0) {
    obj[key]+=', '+currObj[key];
  }
  if (end && typeof obj[key] === 'string') {
    obj[key]=obj[key].split(', ').sort().filter(function(item, pos, self) {
...
function getReadyForFilterObj (obj,result,end){
	for (var key in obj){
		if(obj.hasOwnProperty(key)){
      if (obj[key] instanceof Array) obj[key] = obj[key].join(', ');
...
function excludeNeedlessProps (obj){
  for (var key in obj) {
    ...
    if (obj[key] && obj[key] instanceof Array) {
      index = obj[key].indexOf('выбрать вариант');
    }
    if (!obj[key] || obj[key] == 'выбрать вариант') {
      delete obj[key];
    }
    if (obj[key] && obj[key] instanceof Array && index >= 0) {
